...
// Saves the whitepaper lead and sends the PDF to the user via Mailrelay.
//
...
// - WHITEPAPER_PDF_URL       (required) Public URL of the whitepaper PDF file
...
module.exports = async (req, res) => {
  ...
  const WHITEPAPER_PDF_URL = process.env.WHITEPAPER_PDF_URL || 'https://smartstudent.io/docs/whitepaper-ausentismo-escolar-ia.pdf';
  ...
  const userEmailPayload = {
      ...
      <body style="margin:0;padding:0;background:#f4f6fa;font-family:'Helvetica Neue',Arial,sans-serif;">
        <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;margin-top:20px;margin-bottom:20px;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
          ...
          <div style="background:linear-gradient(135deg,#1e3a5f 0%,#2563eb 100%);padding:40px 30px;text-align:center;">
            ...
            <p style="color:rgba(255,255,255,0.7);font-size:14px;margin:0;">Plataforma de Gestión Escolar con IA</p>
          </div>
          <!-- Body -->
          <div style="padding:40px 30px;">
            <h2 style="color:#0f172a;font-size:22px;margin:0 0 16px 0;">¡Hola ${name}! 👋</h2>
            <p style="color:#4b5563;font-size:16px;line-height:1.6;margin:0 0 16px 0;">
              Gracias por tu interés en nuestro whitepaper sobre ausentismo escolar. 
              Aquí tienes tu copia gratuita:
            </p>
            <div style="text-align:center;margin:32px 0;">
              <a href="${WHITEPAPER_PDF_URL}" 
                 style="display:inline-block;background:linear-gradient(135deg,#2563eb,#3b82f6);color:#ffffff;text-decoration:none;padding:16px 40px;border-radius:12px;font-size:16px;font-weight:bold;">
            ...
            <div style="background:#f0f9ff;border-left:4px solid #2563eb;padding:16px 20px;border-radius:0 8px 8px 0;margin:24px 0;">
              <p style="color:#1e40af;font-size:14px;margin:0 0 4px 0;font-weight:bold;">¿Qué encontrarás en este whitepaper?</p>
              <ul style="color:#4b5563;font-size:14px;line-height:1.8;margin:8px 0 0 0;padding-left:20px;">
                <li>Datos actualizados de ausentismo crónico en Chile</li>
                <li>Cómo la IA predice alumnos en riesgo hasta 3 meses antes</li>
                <li>Protocolos de intervención paso a paso</li>
                <li>Caso real: reducción del 23% en deserción en un semestre</li>
              </ul>
            </div>
            <p style="color:#4b5563;font-size:14px;line-height:1.6;margin:24px 0 0 0;">
              Si tienes preguntas o quieres ver una demo personalizada para <strong>${institution}</strong>, 
              no dudes en responder este correo o 
              <a href="https://smartstudent.io/#demo" style="color:#2563eb;text-decoration:none;font-weight:bold;">agendar una demo aquí</a>.
            </p>
          </div>
          <!-- Footer -->
          <div style="background:#f8fafc;padding:24px 30px;text-align:center;border-top:1px solid #e5e7eb;">
            <p style="color:#9ca3af;font-size:12px;margin:0;">
              © 2026 Smart Student Inc. · Santiago, Chile<br>
    ...
    `,
    text_part: `Hola ${name}, gracias por descargar nuestro whitepaper sobre ausentismo escolar. Descárgalo aquí: ${WHITEPAPER_PDF_URL}`
  };
  ...
  const leadNotifyPayload = {
    from: {
      email: FROM_EMAIL,
      name: 'Smart Student - Leads'
    },
    to: [
      {
        email: LEAD_NOTIFY_EMAIL,
        name: 'Equipo Smart Student'
      }
    ],
  ...
  try {
    ...
    if (!userRes.ok) {
      ...
      return res.status(502).json({ error: 'No se pudo enviar el whitepaper. Inténtalo de nuevo.' });
    }

    // Log the lead (even if we're not using a DB yet, this goes to Vercel Logs)
